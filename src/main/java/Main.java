...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        getPersonAndParents("https://familysearch.org/ark:/61903/4:1:KWCB-QWK", 1, sessionId);
    ...
    private  static void getPersonAndParents(String treePersonURL, int iLevel, String sessionId) {
        List<FamilySearchPlatform> myFamilyTree = new ArrayList<FamilySearchPlatform>(1);

        iAncestorCount++;
        // Read person
        // FamilySearchPlatform treePerson = readGedcomx("https://familysearch.org/platform/tree/persons/KWCB-QWK", sessionId);
        FamilySearchPlatform treePerson = readGedcomx(treePersonURL, sessionId);
            ...
            for (Relationship relationship : treePerson.getRelationships()) {
                if (relationship.getKnownType() == RelationshipType.ParentChild) {
                    ...
                    if (relationship.getPerson2().getResourceId().equals(personID)) {
                        ...
                        FamilySearchPlatform parentPerson = readGedcomx(relationship.getPerson1().getResource().toString(), sessionId);
//                        printPersonSummary(parentPerson, "Parent Data");
                        myFamilyTree.add(parentPerson);
                        if (iLevel < NUMGENS) {
                            getPersonAndParents(FS_ARK_URL_BASE + relationship.getPerson1().getResourceId().toString(), iLevel+1, sessionId);
                        }
                    }
                }
            }
